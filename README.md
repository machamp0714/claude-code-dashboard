...
Grafana: http://localhost:3005 （admin / admin）→ フォルダ "Claude Code"

（ポートを変えたいときは `.env` の `GRAFANA_PORT` を編集して `make reload`）

## データの永続化と停止 / 削除

メトリクス・ログ・Grafana 設定は **ホストの `./data/` 配下（`data/prometheus` / `data/loki` / `data/grafana`）に bind mount で永続化**される。コンテナを止めても消えず、`docker compose down -v` でも消えない（`-v` は named volume を消すが、bind mount のホストディレクトリは対象外）。バックアップは `./data/` をコピーするだけでよい。`./data/` は `.gitignore` 済み。

```bash
make down                       # コンテナ停止（データは ./data に残る）
docker compose down -v          # コンテナ削除（./data のデータは残る）
rm -rf data/                    # ← データを完全に消したいときだけ実行（要注意）
```

## PC 起動時に自動起動する

全サービスに `restart: unless-stopped` を設定済み。Docker デーモンが起動すると（＝コンテナが「明示的に `make down` で止められていない」限り）自動で復帰する。

macOS で PC 起動（ログイン）時に立ち上げるには、**Docker Desktop 自体をログイン時に起動する設定**を有効にするだけでよい：

- Docker Desktop → Settings → General → **「Start Docker Desktop when you sign in to your computer」** にチェック。

これで「ログイン → Docker Desktop 起動 → コンテナ自動復帰」となる。`make down` で明示的に止めた場合は、次に `make up` するまで自動起動しない（`unless-stopped` は明示停止を尊重する）。
...
- **Grafana に `admin:admin` でログインできない（API が 401 を返す）**: `./data/grafana` に過去の起動時の
  admin パスワードが残っており、`.env` の `GF_SECURITY_ADMIN_PASSWORD` と食い違っている場合に起きる
  （`GF_SECURITY_ADMIN_PASSWORD` は Grafana DB の初回作成時にしか反映されない）。
  `docker compose exec grafana grafana-cli admin reset-admin-password admin` でリセットするか、
  Grafana のデータを気にしないなら `make down && rm -rf data/grafana && make up` で作り直す。
